...
import type { OAuthProvider, OnError } from '#auth'
...
export function handleMissingConfiguration(event: H3Event, provider: OAuthProvider, missingKeys: string[], onError?: OnError) {
  const environmentVariables = missingKeys.map(key => `FHIR_OAUTH_${provider.toUpperCase()}_${snakeCase(key).toUpperCase()}`)
