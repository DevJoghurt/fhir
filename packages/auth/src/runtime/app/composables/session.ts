...
import type { UserSession, UserSessionComposable } from '#auth'
...
export function useUserSession(): UserSessionComposable {
  ...
  const sessionState = useState<UserSession>('fhir-session', () => ({}))
  const authReadyState = useState('fhir-auth-ready', () => false)
