...
export function createTerminologies(ctx: FhirProfilingContext) {
	...
	doc.save(ctx.paths.projectPath, ctx.paths.outDir, 'content', '2.terminology', '0.index.md');
	...
	for (const [i, valueSet] of ctx.valueSets.entries()) {
		...
		valueSetDoc.save(ctx.paths.projectPath, ctx.paths.outDir, 'content', '2.terminology', 'value-sets', `${i+1}.${valueSet.id}.md`);
	}
	...
	for (const [i, valueSet] of ctx.valueSets.entries()) {
		...
		valueSetDoc.save(ctx.paths.projectPath, ctx.paths.outDir, 'content', '2.terminology', 'code-systems',`${i+1}.${valueSet.id}.md`);
	}
}